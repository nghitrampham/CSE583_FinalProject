...
DF_TEST_AIR = pd.read_csv("air_pollution_death_rate_related/Data/Air_Pollution/data_air_raw/daily_aqi_by_county_2010.csv")
...
def test_get_the_county_code_air_list():
    ...
    for data in testdata:
        assert type(data) == type(testdata[0])
...
def test_get_the_month_death_data():
    ...
    assert type(res.values[0]) == np.float64
...
def test_find_cor_given_state():
    res1, res2, res3, res4 = correlation_county.find_cor_given_state(DF_TEST_AIR, DF_TEST_DEATH, 2010, 12)
